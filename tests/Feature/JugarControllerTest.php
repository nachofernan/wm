...
// --- Cofres en puntas de brazo (DECISIÓN 035, reparto 037, piso de brazo 1 en 038) ---
// seed 42, 30x30: cofre 0 en (9,4) nivel 2 — ver tests/Unit/Game/MapaBuilderTest.php.
...
test('abrir un cofre válido otorga la gema, graba el índice y registra el evento (035)', function () {
    ...
    $response = $this->postJson("/jugar/{$run->token}/cofre", ['x' => 9, 'y' => 4]);

    $response->assertOk()->assertJson(['ok' => true]);
    expect($response->json('drop.nivel'))->toBe(2); // profundidad de (9,4) en seed 42
